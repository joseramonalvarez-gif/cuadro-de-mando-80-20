...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-white border border-[#E8EEEE] rounded-lg shadow-lg p-3 text-xs">
      ...
      <p className="text-[#3E4C59]">Base: {fmt(data.base)}</p>
      <p className="text-[#3E4C59]">IVA: {fmt(data.value)}</p>
...
export default function VATBreakdownChart({ data }) {
  return (
    <div className="bg-white rounded-2xl p-5 shadow-[0_1px_3px_rgba(27,39,49,0.06)] border border-[#E8EEEE]/60">
      <h3 className="text-sm font-semibold text-[#1B2731] font-['Space_Grotesk'] mb-4">Distribución IVA por Tipo</h3>
          ...
          <PieChart>
            ...
            >
              {data.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={VAT_COLORS[entry.name] || '#B7CAC9'} />
              ))}
            </Pie>
