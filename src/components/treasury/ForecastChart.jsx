...
export default function ForecastChart({ data, period = 30 }) {
  return (
    <div className="bg-white rounded-2xl p-5 shadow-[0_1px_3px_rgba(27,39,49,0.06)] border border-[#E8EEEE]/60">
      <h3 className="text-sm font-semibold text-[#1B2731] font-['Space_Grotesk'] mb-4">
        Previsión de Tesorería — {period} días
      </h3>
      <div className="h-[320px]">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data}>
            <CartesianGrid strokeDasharray="3 3" stroke="#E8EEEE" vertical={false} />
            <XAxis dataKey="date" tick={{ fontSize: 11, fill: '#3E4C59' }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fontSize: 11, fill: '#B7CAC9' }} axisLine={false} tickLine={false} tickFormatter={(v) => `${(v/1000).toFixed(0)}k`} />
            <Tooltip content={<CustomTooltip />} />
            <Legend wrapperStyle={{ fontSize: 11 }} />
            <Line type="monotone" dataKey="entradas" name="Entradas Previstas" stroke="#33A19A" strokeWidth={2} dot={{ r: 3 }} />
            <Line type="monotone" dataKey="salidas" name="Salidas Previstas" stroke="#E05252" strokeWidth={2} dot={{ r: 3 }} strokeDasharray="4 4" />
            <Line type="monotone" dataKey="saldo" name="Saldo Acumulado" stroke="#5BB8B2" strokeWidth={2.5} dot={{ r: 4 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
