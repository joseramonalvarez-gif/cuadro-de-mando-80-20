...
export default function ClientDetailModal({ client, invoices, open, onClose }) {
  if (!client) return null;

  const totalSales = invoices.reduce((sum, inv) => sum + inv.amount, 0);
  const avgTicket = totalSales / invoices.length;

  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-['Space_Grotesk'] text-xl">{client.name}</DialogTitle>
        </DialogHeader>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
          <div className="bg-[#F8F6F1] rounded-xl p-4">
            ...
            <div className="text-xl font-bold text-[#1B2731] font-['Space_Grotesk']">{formatCurrency(totalSales)}</div>
          </div>
          <div className="bg-[#F8F6F1] rounded-xl p-4">
            <div className="flex items-center gap-2 text-xs text-[#3E4C59] mb-1">
        ...
        <div className="mt-6">
          <h4 className="text-sm font-semibold text-[#1B2731] font-['Space_Grotesk'] mb-3">Historial de Facturas</h4>
            ...
            {invoices.map((inv, i) => (
              <div key={i} className="flex items-center justify-between p-3 bg-white border border-[#E8EEEE] rounded-lg hover:bg-[#FDFBF7] transition-colors">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Badge className={`${inv.status === 'Cobrada' ? 'bg-emerald-100 text-emerald-700' : inv.status === 'Pendiente' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'} text-xs`}>
                    {inv.status}
                  </Badge>
                </div>
              </div>
